"""Evaluate UniSRec-based BASG across all 3 seeds (tuning + zero-shot eval).

Reuses existing TemporalExpert checkpoints — no retraining needed.
Tuning/inference only (~5 min/seed); safe to run alongside other GPU workloads.

Usage:
    python scripts/basg/eval_unisrec_basg.py              # all 3 seeds
    python scripts/basg/eval_unisrec_basg.py --seed 2026  # single seed
"""
...
CONFIG = "configs/basg/expert_unisrec.yaml"
...
def main() -> None:
    p = argparse.ArgumentParser(description="UniSRec-based BASG: tune + eval all seeds")
    p.add_argument("--seed", type=int, default=None)
    args = p.parse_args()
    seeds = [args.seed] if args.seed else SEEDS

    print("=" * 70)
    print("  UniSRec-based BASG — Tuning + Zero-Shot Evaluation")
    ...
    m, s = divmod(rem, 60)
    print("\n" + "=" * 70)
    print(f"  UniSRec-based BASG Complete")
    print(f"  Total: {h}h{m:02d}m{s:02d}s")
    print(f"  Failed: {failed}/{len(seeds)*2} stages")
    print(f"  Output: {_PROJECT_ROOT / 'results/mainline/unisrec_basg'}")
    print("=" * 70, flush=True)
